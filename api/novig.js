...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Enrich markets with correct execution prices
    // On Novig exchange, asks are always empty in the orderbook
    // To BUY outcome X: you pay the ASK = 1 - bestBid(other outcome)
    // Because buying X = selling the other side
    const enriched = markets.map(mkt => {
      const book = books[mkt.id];
      const outcomes = mkt.outcomes || [];

      // First pass: get best bid for each outcome
      const bids = {};
      outcomes.forEach(out => {
        const ladder = book?.ladders?.[out.id];
        bids[out.id] = calcBestBid(ladder);
      });

      // Return raw bids — index.html applies the inversion (1 - otherBid) 
      const enrichedOutcomes = outcomes.map(out => ({
        ...out,
        liveBid: bids[out.id],
        price: bids[out.id] || out.last || null,
      }));
