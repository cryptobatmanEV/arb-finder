...
export default async function handler(req, res) {
  ...
  try {
    ...
    const enriched = allMarkets.map((m, idx) => {
      ...
      // Apply 1% fee then ceil to nearest cent — matches Polymarket's actual execution price
      // Confirmed: CLOB 52¢ × 1.01 = 52.52¢ → ceil → 53¢ = actual Polymarket avg fill ✅
      const pmCeil = (p) => Math.ceil(p * (1 + PM_FEE) * 100) / 100;
      const clobYesBuy = yesBase > 0.01 ? pmCeil(yesBase) : null;
      const clobNoBuy  = noBase  > 0.01 ? pmCeil(noBase)  : null;
