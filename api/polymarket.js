 ...
 * Returns full ask/bid ladders with price+size for depth-aware fill simulation.
...
/**
 * Simulate filling a USDC stake across an ask ladder (price ascending).
 * This mirrors how Polymarket computes "Avg Price" for market orders:
 * walks the book level by level, fills as much as possible at each price,
 * returns the volume-weighted average raw price BEFORE fee is applied.
 *
 * @param {Array<{price, size}>} asks  - sorted ascending
 * @param {number} stakeUSDC           - USDC amount to fill (reference size)
 * @returns {number|null}              - weighted avg fill price, or null if no liquidity
 */
function simulateFill(asks, stakeUSDC) {
  ...
  if (totalShares === 0 || totalSpent === 0) return null;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 5. Enrich markets with depth-aware fill simulation + fee
    //    simulateFill() walks the CLOB ladder to get the weighted-average fill price
    //    for a REFERENCE_STAKE order — this is exactly what PM computes as "Avg Price".
