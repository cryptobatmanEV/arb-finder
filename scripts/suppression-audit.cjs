...
function filterSuppressionAudit(cards) {
  const byBook = {};
  APPROVED.forEach(book => {
    const mainCards = cards.filter(card => card.main.some(row => row.platform === book));
    const groupedCards = cards.filter(card => [...card.sideA, ...card.sideB].some(row => row.platform === book));
    byBook[book] = {
      mainLegCards: mainCards.length,
      groupedAvailableCards: groupedCards.length,
      wouldBeHiddenByMainLegOnlyFilter: Math.max(0, groupedCards.length - mainCards.length)
    };
  });
  const active = Object.entries(byBook)
    .filter(([, row]) => row.groupedAvailableCards > 0 || row.mainLegCards > 0)
    .map(([book, row]) => ({ book, ...row }));
  return {
    byBook: active,
    totalAlternateOnlyBookCardAppearances: active.reduce((sum, row) => sum + row.wouldBeHiddenByMainLegOnlyFilter, 0)
  };
}
...
(async () => {
  ...
  const parlayMarkets = parlay.json.markets || [];
  ...
  const output = {
    auditType: 'suppression-audit',
    origin: ORIGIN,
    parlayRaw: {
      count: parlay.json.count ?? parlayMarkets.length,
      countsBySport: parlay.json.countsBySport || {},
      countsByBook: parlay.json.countsByBook || {},
      creditEstimate: parlay.json.creditEstimate ?? null,
      upstreamCallsAttempted: parlay.json.upstreamCallsAttempted ?? null,
      sportPlanning: parlay.json.sportPlanning || null,
      skipped: parlay.json.skipped || {}
    },
    coreRaw: {
      polymarket: (polymarket.json.markets || []).length,
      kalshi: (kalshi.json.markets || []).length
    },
